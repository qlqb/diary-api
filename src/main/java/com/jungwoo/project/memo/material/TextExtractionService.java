...
/**
 * 텍스트 기반 PDF / PPTX에서 전체 텍스트를 추출한다. 스캔 이미지뿐인 PDF(텍스트 레이어 없음)는
 * OCR을 하지 않고 FAILED_NO_TEXT로 명확히 실패 처리한다.
 *
 * <p>IPYNB·HWP·HWPX는 여기가 아니라 {@code material.extract} 패키지가 읽는다 — 그쪽은 전체 텍스트와
 * 단위를 한 번의 파싱으로 함께 만든다. 이 클래스는 기존 PDF·PPTX 동작을 그대로 유지한다.
 */
@Slf4j
@Component
public class TextExtractionService {
    ...
    public ExtractionResult extract(Path filePath, String extension) {
        try {
            String text = switch (extension == null ? "" : extension.toLowerCase(Locale.ROOT)) {
