 ...
 * apply() 시점에만 course_topics(+course_notes)로 확정 삽입된다.
 *
 ...
 *
 * topics는 오직 실제 학습 내용(지식/기술 항목)만 담는다. 담당교수·연락처·수업 운영·평가 비율처럼
 * 학습 대상이 아닌 사실은 topics에 넣지 않고 courseNotes로 분리한다 — LearningMap이 강의계획서
 * 전체를 그대로 펼쳐놓은 화면이 되지 않게 하기 위함이다.
 */
public record MaterialAnalysisPayload(
    ...
    /**
     * 학습 topic이 아닌 과목 사실 정보 한 줄. course(운영 정보) 또는 assessment(평가/일정 정보)로만
     * 분류한다 — course_topics가 아니라 course_notes로 확정된다.
     */
    public record CourseNote(
            /** "COURSE_INFO" 또는 "ASSESSMENT" */
            String category,
            String label,
            String detail
    ) {
