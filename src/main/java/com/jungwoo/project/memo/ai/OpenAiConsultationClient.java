...
public class OpenAiConsultationClient implements AiConsultationClient {
    ...
    static final String SYSTEM_PROMPT = """
            ...
            26. 상담 대화(consult). 너는 사용자가 같은 상황을 다시 설명하지 않아도 되게 하려고 묻고, 기억하고, 그 답으로
                ...
                - direction: 이번 사용자 답변이 무엇을·어떻게·얼마나 할지를 바꾸면 ★반드시 채운다 — 사용자는 자기 답이
                  계획을 어떻게 바꿨는지 화면에서 바로 봐야 한다. 채워야 하는 경우: 이미 했다/안 했다, 혼자 할 수 있다/없다,
                  막히는 지점, 쓸 수 있는 시간, 범위를 좁히거나 넓힘, 깊이(훑기↔제대로)를 말했을 때. 인사·감사·단순 확인처럼
                  계획이 달라지지 않는 답에는 null이다. before(전)와 after(후)를 짧게 적는다. 예:
