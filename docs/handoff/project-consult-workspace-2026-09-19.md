...
8. **버린 초안이 되살아나고, 그 바람에 다른 탭에 들어갈 수 없음**(**기존 결함**, 2026-09-20) — 버리기가 화면 상태만 지웠다.
   서버에는 `PROPOSED`가 남아 있어서, 탭을 옮겨 그 탭의 대화를 다시 읽는 순간 초안이 되살아나고, 되살린 초안이 사용자를 계획 탭으로
   데려가 일정 탭에 들어갈 수 없었다. 고친 것 둘: (1) 버리기를 서버에 쓴다(`POST /api/ai/proposals/{id}/dismiss` → `DISMISSED`,
   되살리기는 원래 `PROPOSED`만 되살린다), (2) **되살린 초안·제안은 화면을 옮기지 않는다**(`tabForProposal(..., {restored})`,
   `handlePeriodPlan`의 restored 분기). 되살렸다는 사실은 AI 패널이 한 줄로 알린다. 마이그레이션은 필요 없다(`DISMISSED`는 이미 있는 값).
