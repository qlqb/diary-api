...
AI 상담도 같은 규칙을 따른다 — 자연어 기간을 이 두 날짜(1~31일)로 번역할 뿐이고,
`AiPlanScope(DAY/WEEK/MONTH/RANGE)`는 그 번역을 검증하기 위한 분류이지 기간의 원본이
아니다. 달력 단위에 맞지 않는 요청은 `RANGE`로 담는다(09 문서 CASE-008 참고).
