...
예산이 `항목 상한 × 120분`을 넘으면 모델을 부르기 전에 400(`PLAN_TARGET_EXCEEDS_ITEM_CAP`)으로
멈춘다. 시간을 부풀리지도 결과를 자르지도 않는다. 7일 이하에서 이게 나오면 계산이 잘못된
것이라 서버 오류 로그를 남기고, 8~31일은 기간을 주 단위로 나누라는 안내다. 자동으로 여러
제안으로 분할하지 않는다.
