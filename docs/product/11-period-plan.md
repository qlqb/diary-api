...
**버린 초안은 되살아나지 않고, 되살린 초안은 화면을 옮기지 않는다.** 열린 초안이 있는지는 서버가 들고 있다 —
화면 상태는 그 사본이다. 그래서 [초안 버리기]는 화면에서 지우는 것으로 끝나지 않고 서버에 쓴다
(`POST /api/ai/proposals/{id}/dismiss` → `DISMISSED`). 되살리기는 `PROPOSED`만 되살리므로, 버린 초안은 탭을 옮기거나
새로고침해도 다시 나오지 않는다. 그리고 **되살린 초안·제안은 사용자가 보던 화면을 옮기지 않는다** — 탭을 누를 때마다
그 탭의 대화가 옛 초안을 되살려 계획 탭으로 끌고 가면 원하는 탭에 들어갈 수 없기 때문이다. 화면을 데려가는 것은 방금
만든 초안뿐이고, 되살린 초안이 있다는 사실은 대화 옆에 한 줄로 알린다. 대화 기록과 그 답으로 바뀐 방향은 남는다 —
버린 것은 초안이지 사용자의 말이 아니다.
