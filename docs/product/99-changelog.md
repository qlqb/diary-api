...
`docs/sql/2026-09-16-material-file-types.sql`(추가 전용·재실행 가능, 로컬 DB에 적용함).

- **자료 형식**: `.hwp`(HWP 5.0)·`.hwpx`·`.ipynb`(nbformat 4)를 자료로 올릴 수 있다. 검증은 확장자 + 파일 앞머리
  시그니처다 — 브라우저의 content type은 형식마다 제각각이라 그 값으로 막으면 정상 파일이 거부된다. 저장하는 값은
  형식별 대표 content type이다.
- **노트북은 셀이 단위다**(`NOTEBOOK_CELL`, 위치 "셀 8~12"). 번호는 원본 순서의 1-based 셀 번호이고 execution_count가 아니다.
  빈 셀을 건너뛰어도 번호는 밀리지 않고, 긴 셀을 나눠도 번호는 그대로다. 실행 결과는 텍스트만 상한 안에서 싣고
  이미지·HTML은 종류만 적는다(코드를 실행하지 않는다).
- **한글 문서는 표를 표로 옮긴다**: `[표 시작] | 3주차 | 9월 25일 | [표 끝]`. 파서의 텍스트 추출기를 그대로 쓰면 행·셀 관계가
  사라져 "9/25"가 어느 주차의 제출일인지 알 수 없다. 쪽수는 파일에 없으므로 `page_count`는 null이고 위치는 "구간 N"이다.
  HWPX는 파서에 넘기기 전에 XML 앞머리의 DOCTYPE·ENTITY 선언을 검사해 외부 엔티티를 차단한다.
- **추출 실패를 이유로 나눈다**(암호·손상·미지원 판·너무 큼·시간 초과). 저장은 됐는데 못 읽은 자료는
  `POST /api/materials/{id}/extraction/retry`로 원본을 다시 읽는다 — 같은 파일을 다시 올리게 하지 않는다.
  상한에 걸려 일부만 읽었으면 `extraction_warning`으로 그 사실을 남긴다.
- **ZIP은 자료가 아니라 가져오기다**: 올리면 내부 목록이 오고, 고른 파일만 각각 독립된 자료가 된다. 확정 뒤의 생성은
  폴러가 하므로 화면을 닫아도 계속되고 새로고침하면 상태를 되찾는다. 확정을 두 번 눌러도, 응답이 유실돼도, 서버가 재시작해도
  자료가 중복 생성되지 않는다(확정·선점 UPDATE 조건 + 자료 INSERT와 항목 완료가 같은 트랜잭션 + UNIQUE material_id).
  경로 이탈·심볼릭 링크·중첩 zip·중복 경로는 고를 수 없고, 해제 크기는 헤더를 믿지 않고 읽으며 센다.
  상한 기본값: 원본 20MiB, 내부 파일 20MiB, 전체 해제 200MiB, 항목 1,000개, 한 번에 100개, 원본 보관 24시간.
- **실사용 결함 수정(같은 날)**: 수업 노트북은 화면 캡처를 마크다운에 `![image.png](data:image/png;base64,...)`로 넣는다
  (실제 파일 한 개에 33장, 셀 하나가 최대 36만 자). 이걸 글자로 읽어 본문이 1MB를 넘었고, 자료 INSERT가 DB의
  max_allowed_packet(1MiB)에 걸려 자료가 만들어지지 않았다 — 화면에는 예외 이름만 보였다. 셀 원문의 base64 그림은
  `[그림: 이름]`으로 바꾸고(그 파일은 1MB → 27KB), 전체 텍스트 사본은 저장 전에 UTF-8 800KB로 자른 뒤 경고를 남긴다
  (분석은 행마다 따로 저장되는 단위를 읽으므로 범위가 줄지 않는다). 가져오기 실패 문구에 예외 이름을 내보내지 않는다.
  확정한 뒤에도 고르지 않고 남겨 둔 파일은 원본 압축이 남아 있는 동안 더 가져올 수 있다.
- **의존성**: `kr.dogfoot:hwplib:1.1.11`, `kr.dogfoot:hwpxlib:1.0.9`(둘 다 Apache-2.0, 전이 의존성 없음 — POI를 자기 패키지로
  옮겨 넣어 기존 poi와 겹치지 않는다), `org.apache.commons:commons-compress:1.26.2`(zip 항목의 UTF-8 플래그·CP949 이름·
  심볼릭 링크 판별. POI가 이미 끌어오던 버전을 명시했다).

## 2026-09-15 — 분석 결과 저장 경합, 「이번만 빼기」 되돌리기, 계획 입력 2단계 선택, 「자세히」 갱신

- **경합**: 모델 응답을 기다리는 동안 자료가 지워지거나 임대가 다른 worker로 넘어간 뒤 응답이 돌아오면 구간·과제 후보·변경안이
  저장되던 문제. `MaterialAnalysisResultWriter`가 작업 행을 `FOR UPDATE`로 잡아 토큰을 대조한 같은 트랜잭션에서만 쓴다. 자료 삭제의
  작업 취소는 `markDeleted` 트랜잭션 맨 앞으로. `MaterialAnalysisResultRaceDbTest`(4)는 수정 전 4건 실패 → 수정 뒤 통과.
- **이번만 빼기·되돌리기**(UI): 되돌리기가 목록만 지우던 것을 고쳐 항목 하나/모두 되돌리기 모두 초안을 다시 요청한다. 기본 AI 경로의
  「이미 알아요」 안내·되돌리기를 호출부로 올려 새 초안이 와도 남는다. 구간만 인용한 항목은 구간의 `parentSourceId`로 학습 항목을 찾는다.
- **계획 입력 2단계 선택**: 과목당 45줄 컷과 우선순위 재정렬 대신 반드시 포함(진행 중·첫 미학습·확정 미완료 과제 연결·사용자 연결) +
  글자 예산(`plan.draft.context-budget-chars-per-course`, 기본 12,000, 실측 약 1.8자/토큰). 판단 경로도 같은 넷은 주차 창 밖이어도
  싣고 확정 과제(마감·완료)를 입력에 넣는다. 과제 수행 자체는 사용자 요청 시에만 항목으로(15번 A16·A17, 13번 컨텍스트 수집 규칙).
- **「자세히」 갱신**: 오래된 안내는 저절로 다시 만들지 않고 [지금 원문으로 다시 만들기]로만 새 판을 만든다(내 메모는 옮김). 메모를
  화면에서 남기고 고친다(PATCH).
- **AI 일일·월간 한도는 상담 호출만 센다**: `ai_usage_logs` 행을 기능 구분 없이 세던 탓에 자료 29개 업로드 직후 백그라운드
  분석 기록 64건만으로 상담이 E429_002로 막혔다(상담 0회). 집계를 `feature = 'AI_CONSULTATION'`으로 좁혔다. 한도를 검사하는 다른
  경로(학습 대화·추천, 자료 분석·연결 제안, 「자세히」, 계획 대화)도 같은 상담 횟수로만 막힌다. `AiUsageLimitServiceDbTest`(2)는 수정 전 2건 실패 → 수정 뒤 통과.

## 2026-09-13 — 자료 자동 분석, 자료 구간·토픽 N:M, 과제 확인·완료, 계획의 자료 입력과 「자세히」

설계는 `15-material-auto-analysis.md`, DB는 `05-database.md §17`, 마이그레이션은
`docs/sql/2026-09-13-material-auto-analysis.sql`(추가 전용·재실행 가능, 로컬 DB에 적용함). 착수 판단은
`docs/handoff/material-auto-analysis-2026-09-13.md`.

- **업로드·기존 자료를 서버가 뒤에서 자동으로 읽는다.** `material_analysis_jobs`가 원본이고 `@Scheduled` 폴러가
  `UPDATE … WHERE` 한 문장으로 선점한다(임대·토큰). 결과 저장은 전부 토큰 대조라 옛 임대의 늦은 결과는 0행이다.
  기존 자료는 틱마다 상한만큼 등록되고 새 업로드가 몰려도 backfill 한 자리는 남긴다. 인증 실패는 전역 쿨다운,
  한도는 백오프, 그 외는 3회. 사용자별 일시중지/재개. 테스트 실행은 worker를 끈다(build.gradle).
- **끝까지 읽는다.** PDF 페이지·PPTX 슬라이드 단위(`material_text_units`)로 청크를 나누고 직전 단위를 겹친다.
  청크마다 구간을 저장하고 checkpoint를 남겨 실패 뒤 성공 구간을 다시 부르지 않는다. 예산을 넘기면 PARTIAL로
  읽은 범위를 적는다. `page_count`는 파일에서 세고 인쇄 쪽수는 확인된 것만 따로 둔다.
- **자료 구간(`material_sections`)**: 역할 복수(설명·예제·문제·제출 요구·일정·운영·정리·참고), 수행 내용, 발췌, 제출 단서,
  날짜 후보(연도 없는 날짜·상대 표현은 isoDate 없이). 삭제된 자료의 발췌는 비운다.
- **토픽 ↔ 구간 N:M(`topic_material_links`)**과 백필(BACKFILL_SOURCE, 280행). 토픽 응답에 `linkedMaterials`·`reviewNote`.
- **자료 정리 변경안(`topic_change_proposals`)**: LINK/ADD/RENAME/MOVE/MERGE/SPLIT를 서버가 검증(순환·없는 id·tempId)하고
  트리 버전으로 낙관적 잠금해 한 트랜잭션으로 적용한다. 이동·이름 변경은 id 유지, 병합·분할은 기록을 복제하지 않고
  `review_note`. 적용 뒤 같은 프로젝트의 다른 열린 변경안은 STALE로 내리고 LINK를 다시 등록한다. 예전 append 경로도
  버전을 올린다.
- **과제(`course_assignments`)**: 제목·마감·완료 체크. "과제인가요?" 후보는 원문에 제출 단서와 제출 어휘가 있고 역할이
  실습·문제·제출일 때만(첫 실행에서 강의계획서의 평가 비율·주차 계획이 전부 후보가 돼 좁혔다). 연도까지 명시된
  제출 날짜만 SOURCE 마감으로 미리 채우고 나머지는 추정 후보. 사용자 답·마감·완료는 version 대조, 재분석이 덮지 않는다.
  오늘 화면은 같은 원본에서 마감 있는 과제만 한 줄로 보여 준다.
- **계획 입력(기본 AI 경로)**: 과목당 앞 30개 대신 진행 중 → 과제 연결 → 첫 미학습부터 순서로 45줄을 남기고, 항목마다
  연결 구간(역할·위치·수행·발췌)을 `[sN]`으로 준다. 확인된 맥락(user_contexts)·과제(확정/확인 전/완료 제외)·
  미연결 구간·미반영 자료 수를 같이 싣는다. 요청 `excludeTopicIds`(이번만), 응답 `pendingMaterials`. 새 출처 종류
  `MATERIAL_SECTION`·`ASSIGNMENT`. 13번 D6은 "서버가 준 구간 중 인용"으로 개정. V1의 `applyUserMarks`는 표식만이
  근거였던 SKIP을 표식이 지워지면 FULL로 되돌린다.
- **「자세히」(`plan_item_details`)**: 항목이 실제로 인용한 구간으로 단계를 만들고 (항목, 근거판)으로 저장·재사용. 시간·마감·
  선택은 불변. 인용 구간이 없으면 만들지 않는다.
- **실호출 검증**(합성 계정·합성 PDF 3종, 모델 gpt-5.6-luna): 3쪽 설명 자료 → 구간 3·후보 0 / 36쪽 뒤쪽에만 실습·제출 →
  2청크·구간 36·후보 1(마지막 쪽, 2026-09-18 SOURCE 마감) / 마감 애매 안내 → 후보 3(상대 표현·연도 없음은 추정, 연도 명시는
  SOURCE). 변경안 적용 200 → 재적용 409, 초안 4항목 전부 refIds(MATERIAL_SECTION 5·ASSIGNMENT 4 인용), 자세히 4단계 생성 후
  재조회 동일 id, 과제 완료 뒤 열린 과제 0. 첫 실행에서 잡은 결함 둘(ADD children의 op 누락 NPE, "S66" 접두어 id 파싱)을
  고쳤다(`ModelJson`, `TopicChangeOpsValidator.normalize`).
- **알려진 한계**: 상담 자료 발췌 3,000자는 그대로. 도서 메타데이터 조회 없음. 스캔 PDF는 추출 실패 표시만.
  실호출 중 devtools가 8080·8081 두 인스턴스에 새 클래스를 함께 올려 worker가 둘 돌았다 — 임대 선점이 중복 없이 나눠 가졌다.


## 2026-09-12 — 확정 중 반복 일정 변경 검증, 동시 미리보기 저장 충돌 (검토 후속)

2026-09-11 작업의 검토(plan-evidence-ux-review-fixes-v1)에서 남은 두 결함의 서버 쪽.

- **확정이 반복 일정도 잠금 조회로 본다.** 확정 트랜잭션은 REPEATABLE READ 스냅샷이라, 확정이
  시작된 뒤 커밋된 수업 시각·요일 변경·보강 이동을 일반 조회가 보지 못했다(실제 DB에서 재현:
  스냅샷 뒤에 옮겨진 수업과 겹치는 조각이 조용히 저장됐다). `RoutineOccurrenceService.expandForUpdate`
  가 루틴 본체·요일·예외를 <b>전부</b> FOR UPDATE로 읽는다 — 본체만 최신이고 요일·예외는 옛
  스냅샷이면 섞인 시간표를 검사하게 된다. 계산은 화면·배치와 같은 `expand`다.
  보장하는 순서: 변경이 먼저 커밋됐으면 확정이 그것을 보고 E409_016으로 거절한다. 확정이 먼저
  잠금을 잡았으면 뒤따르는 변경(수정·예외·새 루틴 INSERT — (user_id, is_deleted) 인덱스 범위의
  next-key 잠금)은 확정이 끝난 뒤 진행되고, 그 뒤는 기존 정책대로다. 잠금은 그 사용자의 행에만
  걸린다(`PlanConfirmRoutineRaceTest`, 래치로 순서 제어).
- **같은 초안의 미리보기 요청을 제안 행 잠금으로 직렬화한다.** 저장이 "조회 후 INSERT"라 겹친
  두 요청이 둘 다 INSERT해 `uq_ai_proposal_schedule_previews_proposal`에 걸려 한쪽이 500이 났다
  (초안 화면이 뜰 때 실제로 그렇게 났고, `SchedulePreviewConcurrencyTest`로 재현됐다). 잠금 뒤의
  요청은 기존 행을 보고 UPDATE한다. 각 요청은 자기 입력으로 계산한 자기 결과를 받는다 — 뒤 요청에
  앞 결과를 돌려주지 않는다.
- 테스트: `PlanProvenanceIntegrationTest`의 "미리보기 그대로 확정" 사례가 배치 결과를 반드시
  내는 픽스처(내일부터 7일, 30분 항목)로 바뀌어 더는 건너뛰지 않고, 시각을 바꾼 비교 사례가 붙었다.
  DDL 변경 없음.

## 2026-09-11 (2차) — 계획 근거를 자료별로 묶고 원문을 바로 연다

같은 날 1차(결함 수정) 위에 얹는 화면 개선의 서버 쪽. "근거 보기"가 학습 항목을 한 줄씩
나열하고 줄마다 프로젝트 버튼을 반복하던 것을, 화면이 <b>학습 범위 / 참고 자료 / AI의 제안</b>
세 묶음으로 보여주고 자료마다 원문 열기 하나를 두게 하려면 서버가 두 가지를 더 남겨야 했다.

- **스냅샷 2판**(`PlanProvenance.SCHEMA_VERSION = 2`). 원본 줄(`ProvidedSource`)에
  `parentSourceId`(생성 당시 구조의 부모 항목)와 `material`(`ProvidedMaterial`: 자료 id·당시
  파일명·MIME·SHA-256·위치 문자열)을 더했다. 둘 다 <b>모델에 준 값이 아니라</b> 서버가 원문
  탐색을 위해 옆에 붙이는 메타데이터라 `providedValue`와 다른 자리에 둔다 — 프롬프트에는
  파일명·해시가 나가지 않는다(캡처 테스트가 본다). 1판 JSON은 그대로 읽히고 늘어난 필드는
  null이다(`PlanProvenanceCodecTest`의 1판 fixture).
- **응답에 자료 파일 상태를 붙인다**(`SourceView.material` = `MaterialView`). 당시 기록
  (origin=RECORDED)과 지금 연결(origin=CURRENT_LINK, 1판 스냅샷)을 구분하고, 상태를
  AVAILABLE / CHANGED(같은 id인데 해시·이름이 다름) / RELINKED(학습 항목이 다른 자료로 다시
  연결됨) / DELETED / UNAVAILABLE로 나눈다. 지금 파일을 당시 원문으로 가장하지 않는다 — 바뀐
  파일은 "원본이 변경됨 · 현재 파일을 열어요"다. 열기 방식은 INLINE(PDF) / DOWNLOAD(PPTX 등)
  / NONE이고, 화면의 버튼 이름이 이것을 따른다. 회차의 자료는 한 번에 찾는다.
- **위치는 문자열이다.** `locator`는 "2주차" 같은 사람이 읽는 값이고 PDF 물리 페이지가 아니다.
  실데이터의 source_locator는 전부 주차 표기라(2026-09-11 기준 271행) 페이지 이동을 만들 근거가
  없다. 화면은 "2주차 확인"으로 안내하고 "해당 부분 열기"를 약속하지 않는다.
- 원문 파일은 기존 `GET /api/materials/{id}/file`(Bearer 헤더, inline)을 그대로 쓴다. 토큰을
  URL에 붙이는 경로는 만들지 않았다.
- DDL 변경 없음. 2판은 같은 JSON 컬럼 안의 필드 추가라 마이그레이션이 필요 없다.

## 2026-09-11 (1차) — 계획 근거 조회·확정 검증의 남은 결함 수정

2026-09-10 출처 추적에서 코드로는 안 보이던 다섯 가지를 고쳤다. 화면 개선(같은 날 2차)과
한 커밋에 섞지 않는다.

- **AI 이동·축소 뒤에도 생성 근거를 찾는다.** 조정 제안(REDUCE/MOVE/DROP)도
  `ai_proposal_items.created_item_id`에 "바뀐 대상"을 남기므로, 최근 행을 집으면 조정 뒤에
  근거가 "기록 없음"으로 바뀌었다. 이제 payload가 CREATE인 행만 원본으로 고르고, 항목 근거의
  회차가 제안의 회차와 다르면 refId를 짝짓지 않는다.
- **확정 직전에 지금 일정과의 겹침을 본다**(`PlanConfirmScheduleGuard`). 미리보기와 확정
  사이에 근무가 생기면 `E409_016`으로 전체를 거절한다 — 겹친 항목만 빼고 확정하면 사용자가
  승인한 계획과 저장된 계획이 조용히 달라진다. 약속과 시각 조각은 FOR UPDATE로 읽어, 아직
  커밋되지 않은 겹치는 INSERT가 끝날 때까지 기다린 뒤 최신 커밋을 본다(래치 동시성 테스트).
  반복 일정은 잠그지 않고 최신 커밋 기준으로만 본다.
- **적용 뒤 직접 수정을 표시한다.** 근거 JSON은 그대로 두고, 조각의 사건 기록에서 주체가
  SYSTEM이 아닌 MOVED/REDUCED/… 를 읽어 `afterApplyChanges`로 내린다. 화면은 조각의
  version이 오르면 들고 있던 응답을 버린다.
- **출처 400줄 상한을 없앴다.** 넘긴 줄을 프롬프트에만 내보내고 스냅샷에서 빼던 경로가
  있었다. 지금은 프롬프트에 나간 줄은 전부 기록하고, 400을 넘으면 경고만 남긴다.
- **DB 테스트 정리.** `PlanProvenanceIntegrationTest`가 실사용 users 첫 행을 쓰고 제목
  패턴(`PRV-%`)으로 사용자 구분 없이 지우던 것을, 전용 사용자 행을 만들고 그 사용자의 행만
  지우는 방식으로 바꿨다. 원본 약속을 실제로 UPDATE한 뒤 스냅샷이 그대로인지도 여기서 본다.
- CI: 빈 DB에서 돌 수 없는 DB 테스트 다섯 개를 `-PexcludeDbTests` 목록에 추가했다
  (목록 밖에 있어 CI가 실패하고 있었다). 로컬 memo DB에서는 전부 돈다.

## 2026-09-10 — 계획 생성 출처 추적: 무엇을 줬고, 이 항목은 무엇을 근거로 하는가

판단(`plan_strategy_json`)은 결론이었다. 그 결론 이전에 모델에게 준 사실들은 프롬프트
문자열로 만들어져 호출과 함께 사라졌고, 그래서 사용자는 "왜 이 항목이 나왔나"는 물을 수
있어도 "무엇을 보고 그렇게 봤나"는 물을 수 없었다. 설계 근거와 범위는 13-plan-judgment.md §10.

- **스냅샷은 프롬프트를 만들면서 같은 자리에서 모은다**(`ProvenanceCollector`). 기록이 줄을
  만들어 돌려주고 호출부가 그 값을 그대로 프롬프트에 쓴다 — 준 것과 남은 것이 갈라질 자리가
  없다. 모델 호출 뒤 DB를 다시 조회해 만들면 그 사이 바뀐 값이 "그때 준 값"이 된다.
  줄 상한에 걸려 안 나간 항목은 기록도 부르지 않으므로 스냅샷에도 없다.
- **서버 계산은 출처가 아니다.** 가용시간 추정과 학습 예산은 `serverCalculations`에 따로
  담고, `inputLineage`로 그 계산의 입력이 전부 남았는지 일부인지 구분한다. 하루 기본 창과
  현재 시각은 가리킬 원본 행이 없어 가용시간 추정은 항상 PARTIAL이다.
- **모델은 회차 안의 인용 번호만 쓴다.** `[s7]` 같은 표식을 각 입력 줄 끝에 붙이고, 항목의
  `refIds`에 그 값만 인용하게 한다. 그 회차에 실제로 제공한 집합에 없는 값은 버리고 개수만
  남긴다 — 버렸다고 모델을 다시 부르지 않는다.
- **세 컬럼 모두 서버 소유다.** `ai_proposals.plan_provenance_json`,
  `ai_proposal_items.evidence_json`, `plan_versions.provenance_json`. 요청 DTO에 이 필드가
  없고, 항목 근거는 사용자가 고친 값이 덮어쓰는 `original_payload`와 다른 컬럼에 둔다.
- **고쳐도 근거를 지우지 않고 상태만 바꾼다**(CURRENT / EDITED_BY_USER / NEEDS_REVIEW).
  다만 배치 미리보기가 정한 시각을 그대로 확정한 것은 사용자 수정으로 세지 않는다 — 그렇게
  했더니 확정한 모든 항목에 "다시 볼 근거"가 붙어 표식이 뜻을 잃었다.
- **적용 이후에도 같은 근거를 본다.** 새 연결을 만들지 않고 기존
  `ai_proposal_items.created_item_id`를 되짚는다. 초안 화면과 계획·오늘 화면이 같은 응답
  (`GET /api/plans/drafts/{proposalId}/provenance`,
  `GET /api/plans/items/{executionItemId}/provenance`)을 쓴다.
- **레거시는 실패가 아니라 사실이다.** 출처 없는 초안도 200이고 `recorded=false`이며, 화면은
  "생성 당시 출처 기록이 없어요"라고 말한다. 지금 DB로 역추정해 채우지 않는다.
- 범위: `plan.draft.generator=AI`(운영 기본값) 경로만. V0/JUDGMENT/V1은 기록하지 않는다.
...
1회성" 요청이 20턴 중 17턴 되묻기로 끝났다. 구조 수정(draft, 다음 항목)과 별개로 모델부터
올리고, 같은 발화를 상향 모델로 돌린 결과를 다음 단계의 비교 기준으로 남긴다.

- **기능별 모델 프로퍼티를 도입했다.** `ai.consultation.model`(비면 전역 fallback,
  `ai.schedule-import.model`과 같은 패턴). 상담 경로(`AiConversationService`)만 이것을 읽고,
  스트리밍 호출과 `ai_usage_logs.model` 기록이 같은 값을 쓴다. `AiConsultationClient`에
  모델을 받는 4인자 `streamTurn` 오버로드를 추가했고 2/3인자는 그대로다.
- **기본값을 바꿨다.** 전역 `spring.ai.openai.chat.model`은 `gpt-5-mini` → `gpt-5.6-luna`,
  상담은 `gpt-5.6-terra`. 코드 안 `@Value` 하드코딩 기본값 9곳도 `gpt-5.6-luna`로 맞췄다.
  둘 다 `/v1/models`에 실재하는 id다(2026-09-08 확인). 상담만 mid 티어인 이유는 한 발화에
  든 여러 일정 요청과 중간에 바뀌는 의도를 추적해야 해서이고, 전역 경로는 구조화 출력과
  서버 검증으로 묶여 있어 소형 모델로 충분하다.
- **베이스라인 재현 스크립트** `scripts/ai-baseline/replay-2026-09-08.py`. 로그의 13개 발화를
  순서대로 보내고 턴별 응답 종류를 표로 남긴다(`--seed`로 로그와 같은 모양의 시간표·근무
  픽스처를 새 계정에 만든다). 상향 모델(terra)로 3회 돌린 결과:

  | run | 턴 수 | ASK 수 | OFFER/PROPOSAL 첫 등장 턴 | ROUTINE 후보 | COMMITMENT 후보 |
  |---|---|---|---|---|---|
  | 1 | 13 | 10 | 1 | 0 | 1 |
  | 2 | 13 | 9 | 1 | 0 | 2 |
  | 3 | 13 | 9 | 1 | 2 | 2 |

  모델을 올려도 13턴 중 9~10턴이 되묻기다. 첫 턴에 근무 전 이동 블록 후보(기존
  `scheduleSuggestions`)는 나오지만 "첫 수업 1시간 전" 루틴은 세 번 중 한 번만, 그것도 화요일
  하나로 나왔다. 원인은 모델 크기가 아니라 확정된 값을 서버가 들고 있지 않는 구조다 — 다음
  항목(draft)이 그것을 고친다. `reasoning-effort=low`/`verbosity=low` 파라미터 거부는 없었다.


## 2026-09-06 (2차) — 판단층과 조각 생성을 붙이고 지표로 판정했다

`13-plan-judgment.md`의 커밋 2~3이다. 커밋 1이 만든 마감 체인 위에 "왜 이렇게 하는가"와
"실제로 무엇을 하는가"를 얹었다.

- **판단(`PlanJudgmentService`)과 조각(`PlanItemService`)을 나눴다.** 실행 기록을 보고
  조각만 다시 만드는 일은 자주 있지만 판단까지 다시 하는 일은 드물다 — 한 호출에 섞으면
  그때마다 판단이 흔들린다. `items:regenerate`가 그 분리의 실효다.
- **서버가 대조하는 것과 모델이 주장하는 것의 경계를 그었다.** 서버는 근거가 실재하는지만
  본다(맥락 행이 있는가, ACTIVE인가 STALE인가, 그 학습 항목이 컨텍스트에 있는가). 없는
  맥락을 가리키는 근거는 버려지고 그 항목은 FULL로 되돌아간다. 실측에서 서버 조정 0건이
  나왔다 — 모델이 근거 없이 압축을 지르지 않았다는 뜻이다.
...
- **되묻기는 코드가 판정한다.** 모델에게 "애매하면 물어봐"라고 맡기면 빈도가 프롬프트를
  손댈 때마다 바뀌고, 사용자는 어제는 만들어 주던 것을 오늘은 되묻는 이유를 알 수 없다.
  그리고 되묻기의 답 중 **저장할 수 있는 것은 「이미 익숙해요」뿐**이다 — 그건
  USER_CONFIRMED 맥락이 되어 다음 판단의 근거가 되고, "처음이에요"는 저장할 상태가 없어
  요청 플래그로만 억제한다. 이 비대칭을 놓쳤을 때 실데이터에서 되묻기가 끝나지 않았다.
- **지표로 판정했다.** `plan.draft.generator`를 V0 / JUDGMENT / V1로 두고 같은 조건에서
  돌려 비교한다. 조각 만드는 코드가 같은 구간끼리 비교하므로 차이가 어디서 왔는지가 분명하다.
  - 커밋 2 게이트(근거 있는 SKIM/SKIP): V0 0 → JUDGMENT 1, 서버 조정 0건
  - 커밋 3 게이트(선수 topic 커버율 / MUST 배치율): 33.3% → 45.8%, 판별 불가 → 7/7
  - 행동 분포도 갈렸다: v0는 구조상 100% READ, V1은 READ 8 · PRACTICE 18 · RECALL 1 · LAB 3
- **측정하다 프롬프트를 두 번 고쳤다.** 46개 전체에 rank를 매기라고 하면 다 비슷한 값이
  나와 아무것도 말하지 않았고(과목당 3개로 제한), 우선순위를 안 알려주면 전부 MUST가 나와
  MUST 배치율이 그냥 배치율과 같은 값이 됐다(분포 규칙 추가). 둘 다 "모델이 말은 하는데
  계획은 안 바뀐다"에 해당하는 문제였다.
- **기본값은 여전히 `AI`다.** V1은 측정용이다. 근거 데이터(`user_mark`·맥락)가 쌓이는
  커밋 4 이후에 다시 재고 그때 채택 여부를 정한다.
- 실측은 한 번씩 돌린 결과다. JUDGMENT 팔은 두 측정에서 37.5%와 29.2%로 갈렸다 — 모델
  호출마다 흔들리므로 한 자리 차이를 근거로 삼지 않는다.

## 2026-09-06 — 실행 조각에 마감 시각을 달고, 모델 없는 v0 생성기를 두었다

판단층(`13-plan-judgment.md`)의 첫 커밋이다. 판단 자체는 아직 없고, 판단이 설 자리와
그 결과가 배치까지 도달하는 경로를 먼저 만들었다.

- **마감 체인이 확정에서 끊겨 있었다.** 제안 payload의 `deadlineDate`는 미리보기까지만
  쓰이고 `execution_items`에는 담을 컬럼이 없었으며, 롤링 배치는 늘 `deadline=null`로
  Timefold를 불렀다. `execution_items.deadline_at`을 만들고 제안 → 확정 → 조각 → 스냅샷 →
  `SchedulingTask.deadline`까지 이었다. Timefold의 HARD 제약은 이미 있던 것을 그대로 쓴다.
  마감 전에 자리가 없으면 넘겨 배치하지 않고 미배치로 남긴다.
- **`deadlineAt`(시각)과 `deadlineDate`(날짜)는 공존하지 않는다.** 수명이 다른 두 값이라
  인수인계 문서의 "둘 다 있으면 우선순위" 전제는 성립하지 않았다. 대신 확정 시
  `deadlineDate`만 있으면 **다음날 00:00**으로 옮겨 담는다 — 미리보기가 쓰던 변환과 같은
  규칙이고, 덕분에 예전부터 있던 날짜 마감도 확정 이후까지 살아남는다.
- **`PlanningContextBuilder`를 만들었다.** 기존 `PeriodPlanDraftGenerator`도 재료를 모으지만
  결과가 곧바로 프롬프트 문자열이 되어 다른 층이 쓸 수 없었다. 구조를 남겨 판단·조각 생성·
  v0가 같은 입력을 공유한다. 새로 들어온 재료는 넷이다 — 학습 항목의 진행 상태, 과목별
  다음 수업 시각, 확정된 맥락, 그리고 날짜가 아닌 **현재 시각**.
- **주차 컬럼을 만들지 않았다.** `course_topics.source_locator`에 `"9주차, 12주차"`,
  `"6 주차"`, `"과목 개요"`가 섞여 있고, `TopicLocators`가 계획 시점에 가장 이른 주차를
  읽는다. 파생값을 컬럼으로 굳히면 자료를 다시 분석할 때 두 값이 어긋난다.
- **`PlanBlockGeneratorV0`(모델 없음)을 두었다.** `plan.draft.generator=V0`로 켠다. 체인의
  실패 원인을 "체인이 끊겼다"와 "모델이 마감을 안 냈다"로 갈리지 않게 하려는 것이고,
  동시에 판단층의 기준선이다 — 사전식 규칙만으로도 과목 순서·수업 전 마감·이미 아는 내용
  제외가 나온다. 판단층이 **추가로** 내야 하는 것은 선수지식 선택·압축·행동 다양화 셋이다.
- **자료 분량 정보가 없다는 사실을 지어내지 않았다.** 분석 payload에 페이지 범위도 유형도
  없어서 v0는 학습 항목 하나를 고정 45분 블록 하나로 만들고, 그 사실을 조각 설명에 적는다.
- **`plan_versions.strategy_json`과 `course_topics.user_mark`를 만들었다.** 아직 v0가 쓰는
  최소 판단만 담기지만, 확정이 초안의 판단을 그대로 옮기는 경로는 완성돼 있다.
  전략은 제안(`ai_proposals.plan_strategy_json`)에 얹혀 있다가 확정 때 복사된다 — 확정
  요청은 전략을 받지 않는다. 클라이언트가 다시 보내면 사용자가 본 판단과 저장된 판단이
  달라질 수 있기 때문이다.
- 실데이터 검증: 7과목 46개 항목을 판단해 30개 블록을 만들었고 전부 마감이 DB까지
  도달했다. 롤링 배치는 11개를 마감 전에 넣고 19개를 미배치로 남겼다.

## 2026-09-05 (2차) — 계획 초안의 출력 토큰 예산을 항목 상한에 맞췄다

"토요일 일요일이랑 다음주까지 일정 짜줘"(9일)가 `AI_GENERATION_FAILED`로 끝났다. 로그에는
Jackson의 `Unexpected end-of-input ... items[5]`만 남아 "모델이 이상한 JSON을 냈다"처럼
보였지만, 실제 원인은 **출력이 토큰 상한에서 잘린 것**이었다.

같은 날 `d5a39d9`가 `MAX_ITEMS`를 15 -> 30으로 올리면서 `ai.planning.max-completion-tokens`를
2,000에 그대로 뒀다. 항목 하나가 제목·`description`(행동 1~3개 + 완료 기준)·`reason`까지
한국어로 150~200토큰이라 30개면 그것만 5,000토큰 안팎인데, gpt-5 계열은 reasoning 토큰까지
같은 상한을 나눠 쓴다. 6번째 항목에서 끊긴 이유다.

- **`ai.planning.max-completion-tokens`를 8,000으로 올렸다.** 이 값은 `MAX_ITEMS`에 묶여
  있다는 것을 프로퍼티 주석과 필드 Javadoc 양쪽에 적었다 — 항목 상한을 다시 만질 때 같이
  움직여야 한다. 상한만 올리고 예산을 두면 같은 방식으로 또 잘린다.
- **잘림을 먼저 잡는다.** `PeriodPlanDraftGenerator.callAi`가 마지막 청크의
  `finishReason`을 보고 `LENGTH`면 파싱을 시도하기 전에 "출력이 토큰 상한에서 잘렸다"고
  로그를 남기며 실패한다. 로그에 상한·`MAX_ITEMS`·실제 출력 토큰을 함께 찍어 다음에는
  스택트레이스를 읽지 않아도 원인이 보이게 했다. 파싱 실패·빈 JSON 로그에도 `finishReason`을
  붙였다.
- **`extractFinishReason`을 `AiChatResponseUtils`로 옮겼다.** `AiConversationService`에만
  있던 헬퍼라 다른 Agent는 이 값을 아예 보지 않고 있었다. `isTruncatedByTokenLimit`도 같이
  둔다.
- **상한 자체(30개)는 건드리지 않았다.** `d5a39d9`의 근거(강도 목표를 30으로 나눠야 항목당
  평균이 프롬프트의 참고 범위 15~90분에 들어온다)는 그대로 유효하다. 잘못된 것은 개수가
  아니라 그 개수를 담을 예산이었다.
- 이번 장애는 이전 항목(기간 계약 통합)이 만든 것이 아니라 **드러낸** 것이다. 그 전에는
  "토일 + 다음 주" 같은 요청이 기간 종류 계약에서 먼저 막혀 이 경로에 9일짜리 계획이 도달한
  적이 없었다.
- 테스트: 백엔드 574건 통과(신규 1건 — `finishReason=LENGTH`로 잘린 응답이 오면 파싱 실패로
  뭉개지 않고 실패하며, 반쪽짜리 계획을 저장하지 않는다).

## 2026-09-05 — AI 계획 기간을 날짜 범위 계약으로 통합했다

2026-09-04(금)에 "이번 주 토일이랑 다음 주까지 계획하고 싶어"(9/5~9/13, 9일)가 통째로
`AI_GENERATION_FAILED`로 끝났다. 모델이 `planScope=WEEK`로 냈고 서버의 `WEEK <= 7일` 계약이
이를 거부했다. **모델의 출력 오류가 아니라 계약의 결함이다** — `DAY/WEEK/MONTH` 셋으로는
하나의 달력 주를 걸치는 정상적인 요청을 담을 값 자체가 없었다.

`AiPlanScope`에 `RANGE`를 더하는 안을 먼저 만들었다가 버렸다. 동작은 하지만 문제를 한 칸
미루는 수정이다 — 사람이 기간을 말하는 방식은 enum보다 빨리 늘어나고, 무엇보다 **같은
기간을 모델이 두 번 판단하는 구조**가 그대로 남는다(OFFER에서 한 번, 버튼을 누른 뒤
CREATE_PROPOSAL에서 또 한 번). 그래서 기간 종류라는 개념 자체를 없앴다.

- **`AiPlanScope`를 삭제했다.** `RANGE` 같은 대체 enum도 만들지 않았다. 계획 기간은 언제나
  실제 날짜 두 개(`periodStartDate`/`periodEndDate`)이고, 상한은 기간형 계획과 같은
  `PeriodPlanDraftGenerator.MAX_PLAN_DAYS`(시작·종료 포함 31일) 하나다. "몇 일짜리 계획인가"는
  서버가 알 필요가 없는 정보였다.
- **기간을 정하는 자리를 OFFER 한 곳으로 옮겼다.** `OFFER_PROPOSAL`은 기간이 필수가 되고
  (기간 없는 OFFER는 계약 위반), `PROPOSAL_READY`는 기간을 쓰는 것 자체가 계약 위반이 됐다.
  요청과 똑같은 값을 적어 보내도 위반이다 — 두 값을 비교하는 자리를 만들지 않으려면 애초에
  쓰지 못하게 하는 편이 확실하다.
- **`OfferAction`이 기간을 들고, 화면이 그 날짜를 보여준다.** 기간을 JSON 안에서만 확정하면
  사용자는 AI가 어떻게 해석했는지 모른 채 버튼을 누른다. 그걸 "확정된 사용자 요청"이라고
  부르는 건 서버 혼자만의 정신승리다. 날짜를 보고 누른 것만 승인이다. 기간 편집 UI는 만들지
  않았다 — 다르면 대화로 고쳐 말하면 새 OFFER가 온다.
- **`AiMessageRequest`에 `periodStartDate`/`periodEndDate`를 추가했다.** CREATE_PROPOSAL에서
  필수이고, AUTO/CREATE_PERIOD_PLAN이 실어 보내면 조용히 무시하지 않고 400으로 막는다 —
  무시하면 어느 단계가 기간을 정하는지가 흐려진다.
- **CREATE_PROPOSAL 프롬프트에 `[확정된 계획 기간]`을 싣는다.** "참고용 오늘 날짜"와 달리
  이건 지시다. 모델은 그 기간 안에서 무엇을 할지만 만든다.
- **검증은 세 가지로 줄었다.** `start <= end`, 1~31일, 기간 전체가 과거가 아닐 것. 앞의 둘은
  `prepareTurn`이 대화방을 잠그기 전에 400으로 막고, "전부 지났는가"는 사용자 시간대가
  필요하므로 턴 실행부가 모델을 부르기 전에 막는다(잘못된 요청에 AI 호출을 쓰지 않는다).
  후보 날짜와 MOVE `toDate`의 범위 검증은 그대로이며, 기준만 "모델이 낸 기간"에서 "요청이
  들고 온 확정 기간"으로 바뀌었다.
- **시작이 과거인 요청은 그대로 둔다.** 수요일에 "이번 주"면 8/31~9/6이 맞는 답이다. 오늘로
  자르지 않는다 — 요청 기간과 배치 가능 구간은 다른 문제이고 후자는 `placementFloor`가 맡는다.
- **`targetDate`는 계획 기간이 아니다.** DATE_ONLY 후보가 놓일 날일 뿐이고, 여러 날짜리
  계획에서 날짜 지정이 없는 후보는 `UNSCHEDULED`여야 한다(프롬프트 규칙 유지). 기간 첫날을
  기본 날짜로 복사하면 예전의 "항목이 첫날에 전부 쌓임" 문제가 다른 문으로 돌아온다.
- **재생(idempotency) 경로의 한계를 남겨 뒀다.** 같은 키 재전송으로 OFFER를 다시 그릴 때는
  그때의 기간을 알 수 없다(`ai_messages`에 저장하지 않는다). 컬럼을 늘리는 대신 가장 좁은
  범위인 오늘 하루로 되돌리고 그 날짜를 카드에 보여준다.
- **이번 범위에서 건드리지 않은 것:** `plan_versions`, 스케줄러 horizon(7일), Timefold,
  계획 강도, 기간 계획(CREATE_PERIOD_PLAN) 경로, DB 스키마·마이그레이션. 기간 해석 전용 AI
  호출도 추가하지 않았다(한 턴 = 모델 호출 1회 유지). DB 변경 없음.
- 테스트: 백엔드 573건 통과(`-PexcludeDbTests` 기준), 프론트 292건 통과. 실제 장애 사례
  (요청 기간 9/5~9/13의 9일 CREATE_PROPOSAL 성공), 31일 성공·32일/역전/전부 과거는 모델을
  부르기 전에 거절, OFFER 기간 필수·한쪽만 채운 기간 위반, PROPOSAL_READY가 기간을 되돌려
  보내면 위반, 후보·MOVE 범위 이탈 거부, 확정 기간이 프롬프트에 지시로 실리는지, OFFER 카드가
  기간을 보여주고 버튼이 그 값을 되돌려 보내는지를 테스트로 고정했다. DB 마이그레이션이
  필요한 `CourseMaterialAnalysisDraftUniquenessTest` 1건은 이번 변경과 무관한 기존 로컬 환경
  이슈이며 CI와 동일하게 제외한 실행에서 전체 통과를 확인했다.
- 문서: `09-ai-consultation-regression-cases.md`의 CASE-008(버린 안과 그 이유, 수동 회귀
  시나리오 포함), `11-period-plan.md`에 AI 상담도 같은 날짜 범위 모델을 쓴다는 한 문장.
...
전부 사라져 손댈 방법이 없었고(유령 항목), 프로젝트는 "보관"이라는 모호한 말만 있었고,
AI 대화는 아예 지울 수가 없었다. **새 보관 시스템·휴지통 화면·복구 센터를 만들지 않았다** —
사용자에게 필요한 것은 "삭제"와 "다시 시작" 두 가지뿐이다.

- **보류 항목에 다시 시작/삭제를 붙였다.** 백엔드에 이미 있던 `POST /api/execution-items/
  {id}/resume`(HOLD -> PLANNED + RESUMED 이벤트)와 soft delete를 그대로 재사용했다 —
  **백엔드 변경 없음.** 프론트에 없던 `executionItemAPI.resume`만 추가했다. 오늘 화면에서
  보류를 완료와 같은 "오늘 정리한 것"에 묻어두지 않고 **"보류한 것"** 영역으로 분리했다
  (보류는 끝난 것이 아니라 되돌릴 수 있는 상태다). DONE/CANCELLED에는 다시 시작을 노출하지
  않는다. 삭제는 행 안에서 한 번 더 확인한다.
- **같은 ExecutionRow를 쓰는 모든 화면에서 동작한다.** 오늘·프로젝트·일정 세 곳 모두 resume/
  delete를 연결해, 오늘 화면에서만 손댈 수 있는 반쪽짜리가 되지 않게 했다.
- **프로젝트 "보관"을 "삭제"로 정리했다.** 백엔드 `DELETE /api/courses/{id}`는 이미
  `status=ARCHIVED` soft delete였고 **그대로 뒀다** — 자료·대화·실행 기록이 이 프로젝트를
  참조하므로 cascade hard delete로 바꾸지 않는다. 프론트만 Archive 아이콘 -> 휴지통,
  문구 -> "이 프로젝트를 삭제할까요?", `courseAPI.archive` -> `courseAPI.delete`로 맞췄다.
  삭제 후 목록을 갱신하고 프로젝트 목록으로 돌아간다. 목록 카드에는 삭제를 중복으로 두지
  않았다 — 프로젝트를 지우는 일은 드물고, 카드 클릭 한 번으로 들어가면 바로 있다.
- **AI 대화 삭제를 추가했다(신규 기능).** `DELETE /api/ai/conversations/{id}`. 내부는
  `status=ARCHIVED` soft delete이고 **DB 컬럼을 추가하지 않았다** — `ConversationStatus`에
  ARCHIVED가 이미 있었고 `chk_ai_conversations_status`도 그 값을 허용한다. 메시지·제안·이미
  승인된 장기 컨텍스트는 지우지 않는다. 목록 쿼리에 `status='ACTIVE'` 조건을 걸어 삭제한
  대화가 다시 나오지 않게 했고, `prepareTurn`이 ARCHIVED 대화를 404로 막아 다른 탭에 열려
  있던 화면이 지운 대화를 되살리지 못하게 했다. 같은 대화를 두 번 지워도 성공으로 본다.
- **대화 목록 행 구조를 바꿨다.** 행 전체가 button이라 삭제 button을 그 안에 중첩해야 했다 —
  container + 열기 button + 삭제 icon button(`aria-label="대화 삭제"`)으로 나눴다.
- **보고 있던 대화를 지웠을 때**: 진행 중 SSE를 끊고, loadToken을 올려 뒤늦은 응답이 화면을
  되살리지 못하게 하고, 활성 id·이력·입력 중이던 글·적용 전 초안을 모두 비운 뒤 목록을 다시
  읽어 남은 대화 중 가장 최근 것을 연다(없으면 새 대화 준비 상태). 목록 화면에서도 삭제 실패
  메시지가 보이도록 오류 표시 위치를 고쳤다(그 전에는 채팅 화면에만 있어 아무 말 없이 남았다).
- 테스트: 백엔드 265건 통과(신규 4건 — 대화 삭제 soft delete/소유권/멱등, 삭제된 대화의 메시지
  전송 차단). 프론트 80건 통과(신규 11건 — 보류 영역 분리, 다시 시작/삭제 액션, 완료·취소에
  다시 시작 미노출, 대화 삭제 4종). build/lint 통과.
- 실제 로컬 서버와 브라우저에서 확인: HOLD -> 다시 시작(`RESUMED` 이벤트, version 1->2),
  HOLD -> 삭제(`is_deleted=1` + `DELETED` 이벤트), 프로젝트 삭제(`courses.status=ARCHIVED`,
  목록·사이드바에서 사라짐), 비활성 대화 삭제(보던 대화 유지), 활성 대화 삭제(이력 사라지고
  남은 대화로 전환 / 마지막 대화면 빈 상태), 삭제된 대화에 메시지 전송 404, 일정 화면의 보류
  항목에도 다시 시작/삭제가 나오는 것까지 확인했다.
