...
## 2026-09-05 — AI 상담이 사용자 지정 기간을 표현할 수 있게 했다
...
- **`AiPlanScope`에 `RANGE`를 추가했다(최대 31일).** 하나의 달력 주/달로 표현할 수 없는
  사용자 지정·혼합 기간이다. 상한 31일은 기간형 계획 도메인(`11-period-plan.md`,
  `PlanDraftService`의 1~31일)과 같은 값이다 — 한 번에 잡는 계획의 상한을 AI 상담만 다르게
  두지 않는다.
- **`WEEK`의 7일 제한은 그대로 뒀다.** `WEEK`를 14일로 늘리는 것이 더 짧은 수정이지만,
  그건 "오늘 계획"이나 "이번 주 계획"이 조용히 열흘로 부푸는 것을 막는 안전장치 자체를
  없애는 일이다. 같은 기간(9/5~9/13)을 `WEEK`로 보내면 지금도 실패한다 — 그 실패를 테스트로
  박아 두어 "그냥 상한을 푼" 구현이 통과하지 못하게 했다.
- **기간의 원본은 계속 `periodStartDate`/`periodEndDate` 두 날짜다.** `planScope`는 그 두
  날짜를 서버가 검증하기 위한 의미 분류이지 기간이 아니다. DTO 필드를 새로 만들지 않았다.
- **`periodViolationReason()`을 if 나열에서 enum switch로 바꿨다.** scope가 늘어나면 컴파일이
  먼저 막는다 — 새 값이 검증 없이 통과하는 경로를 없앴다.
- **프롬프트(원칙 15)에 기간 해석 규칙을 명문화했다.** 여러 표현이 합쳐지거나 사용자가
  시작·종료를 직접 지정하면 `RANGE`, 하나의 달력 주/달을 명시했을 때만 `WEEK`/`MONTH`다.
  기간 길이가 우연히 7일이라고 `WEEK`가 되지 않는다("앞으로 7일"은 `RANGE`).
  **사용자가 명시한 시작점을 앞선 달력 주 월요일로 넓히지 않는다** — "토일부터"는 9/5이지
  8/31이 아니다. 오늘로 잘라내지도 않는다(9/4로 당기지 않는다). 요청 기간과 배치 가능
  구간은 다른 문제이고 후자는 `placementFloor`가 담당한다는 기존 설계를 그대로 따른다.
- **31일 초과는 프롬프트가 `ASK_CLARIFICATION`으로 유도하고 서버도 독립적으로 거부한다.**
  프롬프트=정상 흐름 유도, 서버=최종 방어라는 구조를 유지했다.
- **enum을 늘리지 않았다.** `WEEKEND` / `TEN_DAYS` / `NEXT_WEEK_INCLUDED` 같은 값을 만들기
  시작하면 사람이 기간을 말하는 방식마다 값이 붙는다. 달력 단위에 맞지 않는 것은 전부
  `RANGE` 하나로 받고, 실제 범위는 두 날짜가 말한다.
- **계약 위반 시 AI를 다시 호출하는 재시도는 넣지 않았다.** 이번 실패는 모델 출력의
  흔들림이 아니라 계약의 결함이라 계약부터 고쳤다. 한 턴 = 모델 호출 1회 구조 유지.
- **이번 범위에서 건드리지 않은 것:** `plan_versions`, 스케줄러 horizon(7일), Timefold,
  계획 강도, DB 스키마·마이그레이션, AUTO -> OFFER -> CREATE_PROPOSAL 흐름. DB 변경 없음.
- 테스트: 백엔드 657건 통과(신규 10건 — 실제 장애 사례 `RANGE` 9/5~9/13 성공, 같은 기간을
  `WEEK`로 보내면 실패, `RANGE` 10일/31일 성공·32일 실패, 기간 역전·전체 과거·항목 범위
  이탈·MOVE `toDate` 범위 이탈 실패, 프롬프트 계약 1건). 기존 `DAY`/`WEEK`/`MONTH` 회귀
  테스트는 그대로 통과한다. DB 마이그레이션이 필요한 `CourseMaterialAnalysisDraftUniquenessTest`
  1건은 이번 변경과 무관한 기존 로컬 환경 이슈이며 CI와 동일하게 `-PexcludeDbTests`로 제외한
  실행에서 전체 통과를 확인했다.
- 문서: `09-ai-consultation-regression-cases.md`에 CASE-008(수동 회귀 시나리오 포함),
  `11-period-plan.md`에 AI 상담도 같은 1~31일 기간 규칙을 따른다는 한 문장.
