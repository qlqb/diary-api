...
## 2026-09-21 (후속) — 프로젝트 자료 정리 검토 결함 수정과 남은 요구

직전 구현을 코드 리뷰한 결과로 고쳤다. 설계 15번 §13, DB 05번 §20, API `api-spec.md`,
작업·검증 기록 `docs/handoff/project-tidy-review-2026-09-21.md`. 브랜치 `fix/project-tidy-review`(두 저장소, `dev-playground` 위).
마이그레이션 `docs/sql/2026-09-21-project-tidy-review.sql`은 **사용자 로컬 `memo`에 적용하지 않았다**(추가형 열 셋).

- **편집이 사라지지 않는다**(화면): 저장 주인을 패널 밖으로 옮겼다. 구역 이동·느린 응답·409·재조회에 편집이 날아가던 것을
  막고, 못 보낸 편집은 새로고침을 견딘다(버린 안은 되살리지 않는다).
- **버린 안이 돌아오지 않는다**: 폐기·적용 성공 순간 조회·편집 세대를 올려 늦게 온 응답을 버린다.
- **적용·폐기·교체가 서로 덮지 않는다**: 폐기가 적용 결과(APPLIED)를 DISMISSED로 덮던 경합을 고쳤다. 실제 DB에서 재현했다.
- **옮겨 온 편집은 확인 전에 적용되지 않는다**: 같은 changeId라도 내용이 다르면 확인 필요. 화면·서버 모두 막는다.
- **분석 묶음**: 6번째 묶음이 앞 묶음을 "끝냈다"고 바꾸던 것, 재시도해도 묶음이 다시 열리지 않던 것, 창을 닫아 못 올린 자리가
  영원히 대기이던 것.
- **큰 프로젝트**: 모든 구간을 목록으로 훑고 골라 읽은 뒤 한 번에 판단한다(뒤쪽 자료가 순서 때문에 빠지지 않는다).
- **입력 고정**: 요청 뒤 자료·트리가 바뀌면 몰래 대신하지 않고 멈춘다. 다시 시도는 같은 입력으로.
- **압축**: 확정하면 같은 분석 진행 카드에 들어간다. 이름이 같은 파일은 경로로 구분한다.
- **근거 원문 열기**: PDF는 그 쪽으로, 다른 형식은 위치를 글로. 예전 파일의 발췌·지워진 자료는 그렇다고 말한다.
- **테스트 격리**: DB 테스트가 사용자 `memo`에 붙던 것을 `memo_test`로 갈랐다. 계획·교재 DB 테스트 아홉 개가 실제 계정
  (user_id=1)을 빌려 쓰던 것도 드러났다(격리 DB에는 합성 사용자 하나만 있다).

## 2026-09-21 — 프로젝트별 자료 분석 UX와 통합 정리

자료 여러 개를 올릴 때 **시작 전에 예상 시간**을 보고, 진행률을 보며 기다리거나 다른 화면으로 갈 수 있다(분석 중 추가 업로드도 된다).
학습 구조 정리는 자료 하나하나가 아니라 **사용자가 [이 프로젝트 자료 정리]를 누를 때 그 프로젝트의 자료를 함께 보고 한 번에** 한다.
설계는 15번 §11·§12, DB 05번 §19, API `api-spec.md`(Project Tidy · Material Analysis Batches).
작업·검증 기록은 `docs/handoff/project-tidy-2026-09-21.md`. 브랜치 `feat/project-tidy`(두 저장소, 로컬 `dev-playground` 위).
마이그레이션 `docs/sql/2026-09-21-project-tidy.sql`은 **로컬 `memo`에 적용했다**(§아래 데이터 항목).

- **왜 바꿨나**: 강의 슬라이드가 「스택」을, 교재가 「스택 자료구조」를, 실습 안내가 「스택 구현」을 만든다. 자료별로는 셋 다 옳은
  제안이고 적용하면 중복 항목 셋이 남는다. 게다가 하나를 적용하면 나머지가 옛 트리 기준이 되어 **다시 분석되고**, 새로 나온
  변경안이 또 서로 어긋났다. 합쳐서 순서대로 실행하는 것으로는 풀리지 않는 의미 충돌이라 **함께 보고 한 번에 판단**하게 바꿨다.
- **프로젝트 단위 정리안**: 요청 시점에 입력을 고정(자료 id·제외 사유)하고 비동기로 만든다. 변경은 **영향을 받는 항목**으로 묶여
  보이고(파일별 카드가 아니다), 변경마다 안정적인 `changeId`가 있어 사용자 편집(제목·제외)이 판이 바뀌어도 따라간다.
  실행 순서는 서버가 세운다 — **병합이 맨 뒤**여야 흡수되는 항목의 새 연결이 살아남는 항목으로 옮겨 간다.
- **검토는 트리 변경이 아니다**: 제목·제외는 자동 저장되지만 학습 구조는 [선택한 변경 적용]을 눌러야 바뀐다. 저장 실패·다른 탭의
  선행 저장을 삼키지 않는다(`edit_revision`).
- **적용 불변조건**: 정리안 판·편집 판·트리 판·근거 자료(존재·연결·해시)·인용 구간·선택 의존성을 모두 확인한 뒤 한 트랜잭션.
  상태 전이를 트리 수정 **앞에** 조건부로 둬서 두 탭 동시 적용이 한 번만 반영된다. 근거 자료·연결은 `FOR UPDATE`로 잠근 채
  확인해 검증과 쓰기 사이에 바뀌지 않게 한다. 고른 것이 없으면 트리 판을 올리지 않는다.
- **고정·폐기**: 검토 중 다른 자료가 끝나도 섞지 않고 "새 자료 N개를 반영할 수 있어요"만 알린다. [새 자료 반영해 다시 정리]가
  실패하면 기존 안과 편집이 그대로 남는다. [버리기]는 도는 작업의 임대 토큰까지 올려 **늦게 끝난 결과가 되살아나지 못하게** 한다.
- **업로드 묶음**: 고른 파일의 목록·지원 여부·예상 시간 범위를 먼저 보여주고 [분석 시작]. 진행 상태의 원본은 서버라 탭을 옮겨도
  복원된다. **분석 중 추가 업로드는 새 묶음**이라 기존 진행률이 뒤로 가지 않는다. `processedPercent` 100%는 "처리 종료"이지
  "전부 성공"이 아니며 화면이 "처리 종료 · 4개 성공 / 1개 실패"로 말한다.
- **예상 시간**: `material_analysis_timings`(지난 실행의 실측)에서 계산한다. 산식과 한계(짐작·표본 부족·업로드 시간 제외·압축)는
  `AnalysisEstimator` 주석과 15번 §12.1에 그대로 적었다. 표본이 적으면 화면이 "초기 추정"이라고 말한다.
- **전환**: 자료별 LINK 분석을 등록·실행·저장 세 곳에서 막았다(`material.analysis.link-jobs.enabled`, 기본 false). 열린 자료별
  변경안은 지우지 않고 `SUPERSEDED`로 옮겼다(`superseded_from`에 원래 상태, 롤백 UPDATE는 SQL 파일에). LINK가 곁들여 하던
  과제-프로젝트 연결은 **AI 없는** 별도 단계로 옮겼다. 승인 전 구조 제안 색인과 학습 지도는 원본만 정리안으로 바뀌고 계약은 그대로다.
- 검증: API 1,246건·UI 633건 통과. 실제 모델 3회차 — 같은 개념을 다르게 부르는 자료 셋에서 **자료 수만큼 항목이 생기지 않았고**,
  기존 트리가 있는 상태에서 새 자료는 **기존 항목에 LINK**로 붙었다(ADD 2 · LINK 4). 브라우저(1536×760)에서 고르기→시작→진행→
  검토 수정→적용→버리기까지 클릭으로 확인. 못 본 것은 handoff §4.5·§4.6에 적었다.

## 2026-09-19 — 프로젝트 기반 AI 상담·계획·실행 통합

사용자가 같은 상황을 다시 설명하지 않고 상담을 이어 가며, 답변과 실제 수행 기록이 선택·방법·분량을 바꾸고, 그 이유가 화면에 보인다.
설계는 11번 §5-1-5, 13번 §12, 15번 §4-3·§7.2·§7.3, 10번 §5.1, DB 05번 §18, API `api-spec.md`. 작업·검증 기록은
`docs/handoff/project-consult-workspace-2026-09-19.md`, 화면은 diary-ui `docs/reviews/project-consult-workspace-completion.md`.
브랜치 `feat/project-consult-workspace`(두 저장소, 로컬 `dev-playground` 위). 마이그레이션 둘(`2026-09-19-*.sql`)은 격리 DB에만 적용.

- **전체 범위 탐색**: 펼친 자료 선택이 앞 묶음부터 예산을 다 쓰던 것을 프로젝트·묶음을 돌아가며 허용하도록(기준 코드는 8개 중 2개만
  전달). 대상 프로젝트별 처리 결과(`strategy.projects`) — 보지 못한 것을 제외로 포장하지 않는다.
- **한 회차 예산**: 호출 수 + 입력 토큰 합 + 경과 시간. 선택이 2회를 써도 추가 읽기 1회가 남는다.
- **실제 전달 기록**: `plan_generation_traces`와 `/trace` — 목록 전달·원문 전달·서버 커밋.
- **범위·깊이·시간 분리**: 강도는 필수 질문이 아니다. 합의 kind `DEPTH`·`TIME_BUDGET`, 말한 시간이 초안까지 유지.
- **사용자 이해**: 근거 유형·적용 범위·철회, 상담 턴 자동 저장(인용 확인), 고치기·확인·지우기 API, 점검 활동.
- **상담 턴**: 질문 카드·이해한 내용·바뀐 방향(`consult`), 빠른 답은 저장된 선택지에서만, `PLAN_NOW`.
- **초안**: 최신성(`freshness`), 다시 만들기의 사용자 편집 보존·충돌 안내, 항목의 선정 이유·출처 유형, 운영 안내 항목 방지.
- **자료**: 승인 전 구조 제안의 읽기 전용 색인, 프로젝트 학습 지도, 분석 한도 CONTENT/LINK 분리와 대기열 앞막힘 수정, `.sh`.
- **실행 기록**: 막힌 이유(`blocker_kind`), "측정" → "사용자가 적은 시간".
- 검증: 고정 응답 회귀(T01~T18 대응표는 handoff 문서), 실제 모델 평가 `scripts/ai-baseline/verify-project-consult-2026-09-19.py`.
