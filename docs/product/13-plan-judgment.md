...
> 상태: 커밋 4 완료 (2026-09-07). §8.2 지표 게이트와 커밋 4 왕복 게이트 모두 통과.
> 기본값은 여전히 AI, V1은 측정용
...
| D6 | 모든 조각은 `자료 + 위치` 또는 `실습/회상 행동` 중 하나에 anchor. **자료 위치는 서버가 붙인다** — 학습 항목이 이미 출처를 알고, 모델이 고르면 없는 자료를 지어낼 수 있다. (2026-09-13 개정) 기본 AI 경로에서는 서버가 회차에 실제로 준 자료 구간 `[sN]` 중에서 모델이 인용하고 서버가 대조한다 — 임의 ID 생성은 여전히 없다(15-material-auto-analysis.md A13) | topic 제목만 있는 조각 / 모델이 materialId를 자유 작성하게 하기 |
...
- **학습 항목 창**은 현재 주차 −2 ~ +1. 주차를 모르는 항목은 창 뒤에 과목당 최대 3개.
  (2026-09-15) 창 밖이어도 **진행 중·첫 미학습·확정 미완료 과제 연결·사용자 연결** 항목은 창 뒤에 트리 순서로 붙는다 —
...
**조각만 재생성**(`items:regenerate`)은 판단을 다시 하지 않는다. 대신 재생성 직전에
컨텍스트를 다시 모아 **`user_mark`가 가리키는 항목의 취급만 서버가 SKIP으로 내린다**
(`PlanJudgmentService.applyUserMarks`, `adjustedBy=SERVER` + `USER_MARK` 근거). 내리기만 한다 —
표식을 지웠다고 모델이 냈던 SKIP을 FULL로 되돌리지는 않는다. 그 SKIP에는 다른 근거가 있었을
수 있고 여기서 판단할 방법이 없다(「다시 포함」은 §9 후속). 사용자가 「이미 알아요」로
...
```text
PlanStrategyPanel   "이번 계획은 이렇게 봤어요" — 목표·과목 순서·제외한 내용
PlanAskCard         되묻기. 조각 대신 질문만 뜬다
PlanDraftReview     조각마다 취급·행동·마감·완료 기준과 「이미 알아요」
planLabels.js       enum → 표시 문구. 화면이 이 파일을 지나지 않고는 말하지 않는다
PATCH /api/topics/{id}/user-mark · POST /plans/drafts/{id}/items:regenerate
```

**세 동작이 서로 다르다.** 셋이 같은 일을 하면 사용자는 무엇을 눌러야 할지 알 수 없다.

| 버튼 | 하는 일 | 남는 것 |
|------|---------|---------|
| 이미 알아요 | 표식 저장 → 조각 재생성 | `user_mark=KNOWN`. 다음 계획에도 남는다 |
| 이번 계획에서 제외(체크 해제) | 이번 초안에서만 뺀다 | 없음. 네트워크 호출도 없다 |
| 실행 방법 다시 제안 | 조각만 재생성 | 없음. 판단은 그대로다 |

**판단 영역은 재생성 중에도 그대로 둔다.** 판단이 안 바뀐다는 것을 화면이 보여주는 편이,
안 바뀐다고 글로 적는 것보다 낫다.

**enum 원문을 화면에 내보내지 않는다.** `SKIM`·`PRACTICE`·`MUST`는 서버가 값을 구분하려고
쓰는 이름이지 사용자에게 할 말이 아니다. 번역은 `planLabels.js` 한 곳을 지나고,
`copy.test.js`가 금지어와 enum 노출을 정규식으로 잡는다 — 코드 리뷰로만 지키면 급할 때
새어 들어오고, 한 번 새면 그 표현이 기준이 된다.

**취급과 우선순위는 독립이다.** `꼭 하기 · 핵심만 보기 · 약 20분`이 성립해야 한다 —
한쪽으로 합치면 "중요한데 짧게 본다"를 말할 수 없다.

### 커밋 4 게이트 결과 (2026-09-07)

실DB 왕복 셋을 돌렸다.

| 게이트 | 결과 |
|--------|------|
| 「이미 알아요」 → `user_mark` → 서버 조정 SKIP → 재생성 → 제외 사유 표시 | 통과 (`adjustedBy=SERVER`, 그 항목 조각 없음, 원본 제안 DISMISSED) |
| 「이번 계획에서 제외」가 `course_topics`를 건드리지 않는다 | 통과 (diff 0) |
| ASK → 답 → `USER_CONFIRMED` 맥락 → 같은 사슬에서 재질문 없음 | 통과 (맥락 0 → 1, 2차 되묻기 없음) |

★ 게이트 3은 **처음에 성립하지 않았다.** 되묻기가 한 번도 뜨지 않았고, 원인은 위의
"비율은 그 과목 안에서" 결함이었다. 게이트가 죽은 기능을 잡은 것이고, 고친 뒤 통과했다.

## 6. 다음 커밋

| 커밋 | 내용 | 검증 |
|------|------|------|
(v1 범위는 여기까지다. 남은 것은 §9 후속.)
...
- **UNKNOWN_FAMILIARITY**: 익숙함에 대한 근거(맥락·표식·진행 상태)가 없는 topic이고,
  ...
  ★ **비율은 그 과목 안에서 잰다.** 계획 전체를 분모로 두면 과목이 늘수록 어떤 과목도 25%를
  넘지 못해 되묻기가 사실상 죽는다 — 7과목 계획에서 한 과목이 통째로 근거 없어도 전체의 15%
  남짓이다. 실제로 그렇게 만들어 두고 커밋 4 게이트를 돌렸더니 질문이 한 번도 뜨지 않았고,
  그래서 게이트 3이 성립하지 않았다. 묻는 대상이 한 과목이니 "그 과목의 계획이 답에 따라
  달라지는가"가 맞는 질문이다.
...
- **자료 분량 정보**: (2026-09-13) 자료 구간(material_sections)에 위치·역할·수행 내용이 생겼다. v0의 고정 45분을
  구간 분량으로 바꾸는 것은 아직 하지 않았다
- **「다시 포함」**: (2026-09-13 일부 해결) `applyUserMarks`가 표식만이 근거였던 SKIP은 표식이 지워지면 FULL로
  되돌린다. 다른 근거가 섞인 SKIP은 그대로다 — 그건 재판단의 몫이다. 기본 AI 경로는 표식이 없으면 후보로 다시 들어간다
- **「다시 판단」**: 같은 기간에 판단부터 다시 하는 버튼. 무엇이 바뀌었는지 사용자가 알 수
  있게 diff를 함께 보여줘야 해서 화면이 하나 더 필요하다
- **지연시간 계측**: 판단·조각 호출별 p50/p95. §8.2 표에 채운다
...
★ **한 번 돌린 결과다.** JUDGMENT 팔은 같은 조건에서 **37.5%(1차)와 29.2%(2차)** 로
갈렸다 — V0(33.3%)를 위로도 아래로도 넘는다. 즉 이 지표에서 JUDGMENT와 V0의 차이는 측정
잡음 안에 있고, 둘을 가르는 근거로 쓸 수 없다. V1(45.8%, 앞선 실행 58.3%)만 두 번 다 V0
위였다. 방향이 뚜렷한 것(V1의 커버율, 행동 분포, `topic_id` 연결)만 결론으로 쓴다.

★ **지연시간은 재지 않았다.** 측정 하네스가 벽시계를 기록하지 않았고, 사후에 로그로
복원하면 모델 호출과 DB 왕복이 섞인 값이 된다 — 없는 수치를 지어내지 않는다. 다음 측정
때 호출 단위로 계측해 채운다.
...
## 10. 생성 출처 추적 (2026-09-10)

판단층은 "왜 그렇게 하기로 했는가"를 남긴다. 그 결론 **이전에 무엇을 봤는가**는 어디에도
없었다 — 프롬프트 문자열은 호출과 함께 사라졌다. 그래서 사용자는 "왜 이 항목이 나왔나"는
물을 수 있어도 "무엇을 보고 그렇게 봤나"는 물을 수 없었다.

### 10.1 세 가지를 섞지 않는다

| 무엇 | 어디에 | 화면 문구 |
|---|---|---|
| 모델에 제공한 원본 | `providedSources` | 생성 시 참고한 정보 / 근거로 연결한 정보 |
| 서버가 계산한 것 | `serverCalculations` | 서버가 계산한 것 |
| 모델의 추정·판단 | 항목 근거의 `aiEstimates` / `reason` | AI가 판단한 것 |

셋을 한 목록에 뭉치면 추정이 등록된 사실과 같은 무게로 읽힌다. **"확인됨", "제약 반영 완료",
"배치 확정" 같은 말을 쓰지 않는다** — 인용 번호가 유효한 것과 내용이 맞는 것은 다른 질문이다.

### 10.2 인용은 회차 안에서만 유효하다

모델에는 원본 id를 자유 작성시키지 않고 그 회차의 인용 번호(`[s7]`)를 인용하게 한다. 서버는
그 회차에 **실제로 제공한** 집합만 인정하고 나머지는 버린 뒤 개수만 남긴다(`unknownRefCount`).
버린 것 때문에 모델을 다시 부르지 않는다 — 되묻기 루프를 만들지 않는다.

대조할 것이 "서버가 이미 아는 집합 하나"로 줄어드는 것이 요점이다. 파일명·쪽수를 자유롭게
쓰게 하면 그것이 진짜인지 확인할 방법이 없다.

### 10.3 고친 뒤

근거를 지우지 않고 상태만 바꾼다(`CURRENT` / `EDITED_BY_USER` / `NEEDS_REVIEW`). 고치기 전
근거가 남아야 무엇이 왜 바뀌었는지 읽을 수 있고, 고친 뒤 모델을 다시 부르지 않으므로 "AI가
다시 검토했다"로 읽힐 문구도 만들지 않는다.

★ **배치 미리보기가 정한 시각을 그대로 확정한 것은 사용자 수정이 아니다.** 확정 요청은
미리보기 시각을 그대로 싣기 때문에, 그것까지 수정으로 세면 확정한 모든 항목에 "다시 볼 근거"가
붙어 표식이 뜻을 잃는다(실제 화면에서 그렇게 나왔다). 저장된 미리보기와 대조해 가른다.

### 10.4 범위

`plan.draft.generator=AI`(운영 기본값) 경로만 출처를 남긴다. V0/JUDGMENT/V1은 남기지 않고
화면이 "생성 당시 출처 기록이 없습니다"로 처리한다 — 그쪽은 판단 호출과 조각 호출이 나뉘어
있어, 한쪽만 담은 스냅샷을 "생성 시 참고한 정보"라고 부르면 그 자체가 거짓이 된다. 필요해지면
회차를 둘로 나누는 설계가 먼저다.

### 10.5 화면은 묶어서 보여주고, 원문은 바로 연다 (2026-09-11)

"근거 보기"가 학습 항목을 한 줄씩 나열하고 줄마다 프로젝트 버튼을 반복하면, 원문을 확인하려고
프로젝트로 건너가 다시 찾게 되고 긴 내부 스크롤 때문에 검토 중이던 계획을 놓친다. 그래서 기본
화면은 세 묶음이다: **학습 범위**(과목·주차는 전부 같을 때만 한 번, 부모 아래 자식) / **참고
자료**(자료마다 파일명 · 확인할 위치 · 열기 하나) / **AI의 제안**(이유·예상 시간). 개별 인용
줄과 회차 전체 정보는 "생성 당시 정보 자세히 보기" 안에 그대로 남는다 — 줄이는 것은 반복이지
근거가 아니다.

묶는 규칙은 화면이 아니라 데이터가 정한다.

- 자료 id로 묶는다. 제목이 같아도 id가 다르면 다른 자료다.
- 부모·자식은 스냅샷의 `parentSourceId`로만 잇는다. 문자열이 비슷하다고 계층을 만들지 않고,
  부모가 그 항목에 함께 인용되지 않았으면 자식을 부모 아래로 끌어오지 않는다.
- 위치는 목록이다. "3쪽"과 "9쪽"을 "3~9쪽"으로 만들지 않는다. 주차가 다르면 통일하지 않는다.
- 그 항목에 실제로 연결된 ref만 본다. 회차에 제공된 나머지는 그 항목의 근거가 아니다.
- 새 문장을 만들지 않는다. 화면의 글자는 전부 저장된 항목명·파일명·위치·AI 판단이다.

버튼 이름은 실제로 일어나는 일이다: PDF는 "원본 자료 열기"(새 탭, 검토 상태 유지), PPTX처럼
브라우저가 그리지 못하는 파일은 "파일 내려받기", 열 수 없으면 버튼 대신 이유("원본 자료가
지워졌어요"). 원문이 없을 때만 "프로젝트 열기"가 보조 경로로 붙는다. 위치는 "2주차 확인"으로
안내하고 페이지 이동을 약속하지 않는다 — 실데이터의 위치가 전부 주차 표기라 물리 페이지를
아는 경우가 없고, 앱 내장 브라우저에서는 새 탭 열기를 확인할 수 없어 "해당 부분 열기"를
구현하지 않았다(4.3 계약의 낮은 단계).

당시 기록과 지금 파일을 섞지 않는다. 같은 id인데 해시가 다르면 "원본이 변경됨 · 현재 파일을
열어요", 학습 항목이 다른 자료로 다시 연결됐으면 "생성 당시와 다른 자료가 연결돼 있어요", 1판
...
## 12. 사용자 이해와 실제 전달 기록 (2026-09-19)

### 12.1 "AI가 이해한 내 상황" — 근거 유형·적용 범위·철회

`user_contexts`는 문장과 상태뿐이었다. 직접 진술·자기평가·관찰·추정이 구분되지 않아 AI의 추정이 사실처럼 계획에 들어가거나,
한 프로젝트에서 한 말이 전체로 일반화될 수 있었다.

| 근거 `evidence_type` | 뜻 | 만들 수 있는 주체 |
|---|---|---|
| STATED | 사용자가 직접 말했거나 직접 고쳤다 | 상담 턴(인용 확인 통과), 사용자 편집·확인 |
| SELF_REPORT | 자기평가("알아·애매해·처음 봐", "혼자는 못 해"). 숙달의 증거가 아니다 | 상담 턴(인용 확인 통과), 점검 활동 |
| OBSERVED | 서버가 실행 기록에서 확인한 관찰 | 서버만(모델이 주장하면 INFERRED로 낮춘다) |
| INFERRED | AI 추정. 확인 전에는 사실이 아니다 | 상담 턴 |

- 적용 범위: `course_id`·`topic_id`·`section_id`·`scope_start/end`. 프롬프트에는 `(자기평가 · 프로젝트 #12 한정 · 2026-09-19~2026-09-20 한정)`
  처럼 붙고, 계획 프롬프트는 "자기평가는 혼자 해낸 증거가 아니다 / AI 추정은 가정으로만 / 범위 밖으로 일반화하지 않는다"를 함께 말한다.
- 상담 턴이 **바로 저장**한다(승인 팝업 없음, `source_type=CONSULT_AUTO`). 서버가 지키는 것: STATED·SELF_REPORT는 `quote`가 이번 사용자
  발화 안에 있어야 한다 — 없으면 INFERRED. AI의 말이 사용자 동의 없이 사용자 상태가 되지 않는다. 한 턴 최대 4개, 남의 프로젝트 id 거부.
- 고치면 옛 행은 SUPERSEDED로 남고 새 행(STATED, `USER_EDITED`)이 생긴다. 지우면 **WITHDRAWN** — 같은 내용은 다시 저장하지 않고
  (서버가 거절), 상담 프롬프트에 `[사용자가 지운 기억]`으로 실어 대화 기록에서 되살아나지 않게 한다.
- 변경 응답의 `staleDraftIds`는 그 기억을 근거(USER_CONTEXT 출처)로 든 **열린** 초안이다. 적용된 일정은 승인 없이 바뀌지 않는다.
- "이미 했어"라는 진술은 기억으로 저장해 계획 판단에 쓰되, 실행 항목의 완료 상태는 바꾸지 않는다(완료는 명시적 기록 동작).
- 단일 퍼센트 숙련도나 사용자 전체 등급은 도입하지 않았다.

### 12.2 상담 턴의 구조화 출력 `consult`

질문 답변을 저장만 하고 끝내지 않는다 — 무엇을 알게 됐고 무엇이 바뀌었는지 남긴다. `ai_messages.consult_json`에 저장해
새로고침·재접속 뒤에도 같은 카드가 복구된다.

- `question`: 한 번에 한 주제. 질문 수의 고정 상한은 없다. 선택지는 제안일 뿐이고(모든 원인을 열거한 것이 아니다) 화면이
  둘 다 아님·잘 모르겠어·직접 말하기·건너뛰기를 항상 함께 준다. 빠른 답(`answer{questionId, choiceIds, skipped}`)은 **저장된 질문의
  선택지에서만** 사용자 발화를 만든다(클라이언트 라벨을 믿지 않는다) — 선택 답과 자유 답은 같은 경로이고 둘 다 대화 기록에 남는다.
- `understanding`: 이번 턴에 저장한 기억(MEMORY)과 이번 발화에서 생긴 합의(BRIEF). 같은 말이 양쪽에 남으면 한 번만 보인다.
- `direction`: 답이 무엇을·어떻게·얼마나 할지를 바꾸면 전 → 후와 이유. 읽지 않은 자료의 파일명·문제 조건은 넣지 않는다.
  `affectsDraft`면 화면이 열린 초안을 "갱신 필요"로 표시한다.
- `requestedAction=PLAN_NOW`: "지금까지 얘기로 계획해줘". AUTO 턴으로 처리하되 남은 질문은 가정으로 돌린다.
- 적용된 계획이 있어도 사용자가 "새 계획"·"무시하고 진행"이라고 했으면 같은 확인을 두 번 하지 않는다. 단 "새 계획"은 영구 제외와 고정
  일정까지 무시하라는 뜻이 아니다.

### 12.3 실제 전달 기록 `plan_generation_traces`

2026-09-19 진단의 "156/367만 보였다"는 추정이었다 — 선택 호출의 입력을 남기지 않았기 때문이다. 이제 회차의 호출마다
실제로 보낸 시스템·사용자 프롬프트(불변 스냅샷), 그 입력에 **줄로 실린** 구간·학습 항목 id, **원문으로 실린** 구간 id, 프로젝트별
집계(후보/목록/선택/원문), 실행 서버 커밋(`BuildVersion` — 개발한 기능과 실제로 돈 서버를 구분), 모델, 추정 토큰을 남긴다.
모델의 비공개 사고과정은 저장하지 않는다.

- `GET /api/plans/drafts/{id}/trace` — 소유자만. 기본은 id·집계·해시만, `includeText=true`일 때만 전문.
- 전문에는 자료 원문이 있다. **자료를 삭제하면 그 자료가 실린 행의 전문을 지운다**(`material_ids`로 찾는다, id·집계는 남는다).
  `plan.trace.retention-days`(30)보다 오래된 행은 새 기록을 쓸 때 지운다. 실패한 회차도 남긴다(무엇을 보냈는지가 더 필요하다).
- 출처 스냅샷(`plan_provenance_json`)에는 `PROJECT_OUTCOMES` 계산(프로젝트별 결과 + 서버 커밋)이 추가됐다.
