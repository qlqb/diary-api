...
| D6 | 모든 조각은 `자료 + 위치` 또는 `실습/회상 행동` 중 하나에 anchor. **자료 위치는 서버가 붙인다** — 학습 항목이 이미 출처를 알고, 모델이 고르면 없는 자료를 지어낼 수 있다 | topic 제목만 있는 조각 / 모델이 materialId를 고르게 하기 |
...
- **「다시 포함」**: `user_mark`를 지운 뒤 그 항목을 이번 계획에 되돌리는 동작. 지금은
  표식을 지워도 이미 SKIP인 취급이 그대로다 — 되돌리려면 그 SKIP의 다른 근거를 다시
  판단해야 하고, 그건 조각 재생성이 아니라 재판단이다
